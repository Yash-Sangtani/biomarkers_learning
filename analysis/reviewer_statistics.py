...
# Primary data: results-video-break has 7 participants (incl. Navya)
PARTICIPANT_INFO_DIR = os.path.join(PROJECT_ROOT, "results-video-break", "participant_info")
# Secondary data: top-level participant_info has ECG metrics for 6 participants
ECG_PARTICIPANT_DIR = os.path.join(PROJECT_ROOT, "participant_info")
...
def load_participant_data(folder_path=None):
    """Load all participant JSON files, merging ECG data from secondary source."""
    ...
    # Merge ECG data from the secondary directory (participant_info/)
    # The primary dir (results-video-break/) has more participants but lacks ECG.
    # The secondary dir (participant_info/) has ECG for 6 participants.
    if os.path.isdir(ECG_PARTICIPANT_DIR):
        ecg_lookup = {}  # (participant_name, video) -> ecg_metrics
        name_to_id = {}  # participant_name -> id from ECG dir
        for filename in sorted(os.listdir(ECG_PARTICIPANT_DIR)):
            if not filename.endswith(".json") or filename.startswith("."):
                continue
            pname = filename.replace("_new.json", "").replace(".json", "")
            filepath = os.path.join(ECG_PARTICIPANT_DIR, filename)
            with open(filepath, "r") as f:
                ecg_data = json.load(f)
            for record in ecg_data:
                vid = record.get("video", "")
                ecg = record.get("ecg_metrics", {})
                if ecg:
                    ecg_lookup[(pname.lower(), vid)] = ecg

        # Build a name lookup from the primary data filenames
        primary_name_map = {}
        for filename in sorted(os.listdir(folder_path)):
            if filename.endswith(".json") and not filename.startswith("."):
                pname = filename.replace("_new.json", "").replace(".json", "")
                ...
                with open(filepath, "r") as f:
                    records = json.load(f)
                if records:
                    primary_name_map[records[0].get("participant_id")] = pname.lower()

        # Merge ECG into the primary data
        for record in participants_data:
            if not record.get("ecg_metrics"):
                pid = record.get("participant_id")
                pname = primary_name_map.get(pid, "")
                vid = record.get("video", "")
                ecg = ecg_lookup.get((pname, vid))
                if ecg:
                    record["ecg_metrics"] = ecg
...
def extract_features(participants_data):
    ...
    for vd in participants_data:
        ...
        # Fixations: empty, absent, or all-zero
        fixation_total = 0
        if isinstance(fixations, dict):
            for dur_data in fixations.values():
                if isinstance(dur_data, dict):
                    fixation_total += sum(dur_data.values())
        if not isinstance(fixations, dict) or not fixations:
            for k in EYE_FEATURES:
                if "fixation" in k:
                    ey[k] = None
            missing_reason.setdefault("fixations", "no_samples")
        elif fixation_total == 0:
            for k in EYE_FEATURES:
                if "fixation" in k:
                    ey[k] = None
            missing_reason.setdefault("fixations", "zero_fixations_detected_possible_tracker_loss")
