...
from .resnet_multibranch import BasicBlock, Bottleneck, ResNet
...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(self.num_branches):
            output_classes = (
                self.num_classes if i is self.num_branches - 1 else self.num_classes - 1
            )
            self.classifiers.append(
                nn.Linear(self.in_planes, output_classes, bias=cfg.BiasInCls)
            )
        ...
        if cfg.train_mode == "sp_confidence":
            ...
            self.project_w = nn.Sequential(
                nn.Linear(input_dim, cfg.num_classes),
                nn.PReLU(),
                nn.Linear(cfg.num_classes, 1),
            )
    ...
    def forward(self, x, label=None, training_phase=None, c=None):
        x_final, x_list, softlabel_list, targets, inds, inds_softlabel = self.base(
            x, label, training_phase=training_phase, c=c
        )
        ...
        softlabel_list = (
            [self.bottleneck(_x) for _x in softlabel_list]
            if self.neck
            else softlabel_list
        )
        ...
        if training_phase == "sp_confidence":
            ...
            softlabel_list = [
                self.classifiers[i](softlabel_list[i])
            ...
            one_hot = (
                torch.zeros(label.shape[0], self.num_classes)
                .cuda()
                .scatter_(1, label, 1)
            )
            # cal mean according to different classes
            cos_matrixs = [
                cos_similarity[:, ind[:, 0]] for ind in inds_softlabel
            ]  # may be change by torch.Tensor.indexadd(dim, index, tensor)
            cos_matrixs_mean = [
                torch.mean(cos_m, dim=1, keepdim=True) for cos_m in cos_matrixs
            ]
            cos_mean = torch.cat(tuple(cos_matrixs_mean), dim=1)  # bs x num_class
            # scale to 0-1, the effect is minor
            cos_mean = (cos_mean - cos_mean.min(dim=1, keepdim=True)[0]) / (
                cos_mean.max(dim=1, keepdim=True)[0]
                - cos_mean.min(dim=1, keepdim=True)[0]
            )
    ...
    def load_param(self, cfg):
        ...
        param_dict = torch.load(
            pretrained, map_location=lambda storage, loc: storage.cpu()
        )
