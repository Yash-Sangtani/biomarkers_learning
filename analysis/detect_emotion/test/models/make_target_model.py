...
from .resnet_ibn_a import Bottleneck_IBN, ResNet_IBN
...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        if model_name == "resnet18":
            ...
            self.in_planes = 2048
            self.base = ResNet_IBN(
                last_stride=last_stride, block=Bottleneck_IBN, layers=[3, 4, 6, 3]
            )
        else:
        ...
        self.classifiers.append(
            nn.Linear(self.in_planes, self.num_classes, bias=cfg.BiasInCls)
        )
            ...
            for i in param_dict:
                if "layer4" in i:
                    if "layer4_{}".format(self.num_branches - 1) in i:
                        j = i.replace("module.", "")
                        j = j.replace(
                            "layer4_{}".format(self.num_branches - 1), "layer4"
                        )
